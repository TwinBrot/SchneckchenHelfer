		...
		try {
			...
			if (event.getNewValue().getId().equals(VoiceID)) {
				...
				if (!LogChannel.equals("")) {
					...
					voicekick.addField("User aus Voice gekickt", event.getMember().getEffectiveName()
							+ " wurde in den AFK Channel verschoben und deswegen von mir aus dem Voice gekickt!",
							false);
