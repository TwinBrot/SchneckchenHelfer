...
public class CatBoy extends ListenerAdapter {
	...
	public void onPrivateMessageReceived(PrivateMessageReceivedEvent event) {
		try {
			...
			try {
				ini = new Wini(new File(Main.Pfad + "catboy.ini"));
			} catch (Exception e) {
				File newFile = new File(Main.Pfad + "catboy.ini");
				try {
			...
			if (args[0].equalsIgnoreCase("!cat") && (event.getAuthor().getId().equals("227131380058947584"))) {
				...
				if (args[1].equalsIgnoreCase("add")) {
					...
					try {
						EmbedBuilder add = new EmbedBuilder();
						add.addField("Bild entfernt:",
								"**Link:** " + ini.get("Images", args[2]) + "\n**ID:** " + args[2], false);
					...
					for (int p = 0; p <= ((ini.get("Images").size() / 24)); p++) {
						...
						list.setAuthor("catboy Image List");
			...
			// BEFEHL
			if (event.getMessage().getContentRaw().equalsIgnoreCase("!catboy")) {

				Random random = new Random();
				int rand = random.nextInt(ini.get("Images").size());
				String catboy = ini.get("Images", rand + "");
				event.getChannel().sendMessage(catboy)
						.queue();
