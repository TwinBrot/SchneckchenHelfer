...
public class Ban {
	...
	public static void onSlashCommand(SlashCommandInteractionEvent event, User user, Member member, String Text, InteractionHook hook,
			@Nullable DateTime unbantime) {
		try {
			if (!(member == null)) {
	...
	private static void ban(Member modmember, Guild guild, Member banmember, User banuser, String text, InteractionHook hook, Boolean dm,
			DateTime unbantime) {
		// User Bannen
		String reasontrim = Discord.trim(text);
		guild.ban(UserSnowflake.fromId(banuser.getId()), 7, reasontrim).queue(success -> {
			try {
				EmbedBuilder builderintern = Discord.standardEmbed(Color.GREEN, "User wurde vom Server gebannt", banuser.getId(),
						banuser.getEffectiveAvatarUrl());
